...
fprintf(' -> PD algorithm with 1P2D (Bregman distance).\n');
param.Algorithm     = 1;
...
fprintf(' -> PD algorithm with 2P1D (Bregman distance).\n');
param.Algorithm     = 2;
param.adaptStepSize = 0;
time1b       = tic;
...
fprintf(' -> PD algorithm with 1P2D (Augmented Lagrangian (FISTA(2)).\n');
param.Algorithm     = 3;
...
%% Call the solver (Algorithm 4a).
fprintf(' -> PD algorithm with 2P1D (Augmented Lagrangian (FISTA(2)).\n');
param.Algorithm     = 4;
param.InnerMaxIters = 2;
param.adaptStepSize = 0;
time1d       = tic;
[x1d, out1d] = decoptLassoSolver('BP', A, b, param, 'x0', x0, 'groups', groups);
time1d       = toc(time1d);
...
fprintf(' -> PD algorithm with 1P2D (Augmented Lagrangian (FISTA(5)).\n');
param.Algorithm     = 3;
...
fprintf(' -> PD algorithm with 2P1D (Augmented Lagrangian (FISTA(5)).\n');
param.Algorithm     = 4;
param.InnerMaxIters = 5;
param.adaptStepSize = 0;
time1f       = tic;
[x1f, out1f] = decoptLassoSolver('BP', A, b, param, 'x0', x0, 'groups', groups);
time1f       = toc(time1f);
...
rmfield(out1f, 'auxi'); 
%%
save_file = ['p02062014_05noise_', num2str(m), '_', num2str(n), '_', num2str(ng), '_', num2str(pid), '.mat'];
save(save_file, '-v7.3'); 
