function g = mce(img1, simpleExtra, MVx, MVy, SearchLimit)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[M N C]     = size(f_prev);
L           = floor(BlockSize/2);
BlockRange  = 0:BlockSize-1;
xc_range    = SearchLimit+1 : BlockSize : N-(SearchLimit);
yc_range    = SearchLimit+1 : BlockSize : M-(SearchLimit);
...
g = g_mce( ((BlockSize/2)+1):(((BlockSize/2)+1)+m-1), ((BlockSize/2)+1):(((BlockSize/2)+1)+n-1));
g_map = map( ((BlockSize/2)+1):(((BlockSize/2)+1)+m-1), ((BlockSize/2)+1):(((BlockSize/2)+1)+n-1));
g = g./g_map;
g = coverAreas(g, img1, simpleExtra);
