...
BlockSize   = max(opts.BlockSize,   4);
SearchLimit = max(opts.SearchLimit, 2);
...
for i = 1:length(yc_range)
    for j = 1:length(xc_range)
        ...
        Block           = img_ref(yc + BlockRange, xc + BlockRange, :);
        
        % Choose either one of the followings
         [MVy1 MVx1]     = FullSearchnew(Block, img_test, xc, yc, SearchLimit);
