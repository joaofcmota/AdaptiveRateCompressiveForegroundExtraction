function [] = ExperimentsTrackingImagesFFT(Algorithm_parameters, Execution_parameters)
...
try
    ...
    fprintf('Some of the parameters of the input of the function ExperimentsTrackingImages were not set correctly')
    return
end
...
CS_measurements_im1 = ceil(2*s1*log(n/s1) + (7/5)*s1 + 1);
CS_measurements_im2 = ceil(2*s2*log(n/s2) + (7/5)*s2 + 1);
% **********************************************************
...
A = createOperatorFFT(CS_measurements_im1, n);

% Background measurements
y_b = A(vec(Im_backgr), 1);
...
A = createOperatorFFT(CS_measurements_im2, n);
...
f_3_diff = spgl1(A, y_3 - y_b, 0 , 0, vec(Im_backgr), opts_spgl1);
...
for kk = 3 : nFrames
    ...
    m_theory = ceil(m_theory);
    
    numMeasurementsL1L1_oracle(kk) = m_theory;
    
    numMeasurementsCS_oracle(kk) = 2*sk*log(n/sk) + (7/5)*sk + 1;

    fprintf('CS   measurements (oracle) = %d\n', round(numMeasurementsCS_oracle(kk)));
    ...
    A_k = createOperatorFFT(m_k, n);
    
    y_k = A_k(vec(imStack(:,:,kk)), 1);
    y_b = A_k(vec(Im_backgr), 1);
            
    % Reconstruct difference of frames using L1-L1 minimization    
    A_handler  = @(x) A_k(x, 1);
    AT_handler = @(y) A_k(y, 2);
    f_k_diff = solverL1L1_name(y_k - y_b, w, 1, 2, A_handler, AT_handler, ...
        MAX_ITER_L1L1);
...
if SHOW_IMAGES
    ...
    figure(2);clf;
    plot(num_measurements,'b-');
    hold on;
    plot(numMeasurementsL1L1_oracle,'k-');
    plot(L1L1_boundEstimate,'g-');
    plot(numMeasurementsCS_oracle,'r-');
